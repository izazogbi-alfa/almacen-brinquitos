...
const BLOB_PATH = "almacen-brinquitos/usuarios.json";
const GITHUB_FILE = "data/usuarios.json";
...
export function archivoUsuariosLocal() {
  if (process.env.VERCEL) {
    return join("/tmp", "almacen-brinquitos", "usuarios.json");
  }
  return join(process.cwd(), "data", "usuarios.json");
}

export function archivoUsuariosEmpaquetado() {
  return join(process.cwd(), "data", "usuarios.json");
}

export function leerUsuariosArchivo(path: string): UsuariosPersistidos | null {
  if (!existsSync(path)) return null;
  try {
    return parseUsuariosPersistidos(JSON.parse(readFileSync(path, "utf8")));
  } catch (error) {
    console.error("usuarios file read failed", path, error);
    return null;
  }
}

export function escribirUsuariosArchivo(
  path: string,
  data: UsuariosPersistidos,
) {
...
function tokenUsuarios(data: UsuariosPersistidos) {
  const packed = gzipSync(Buffer.from(JSON.stringify(data), "utf8")).toString(
    "base64url",
  );
  return `brq2.${packed}.${hmac(packed)}`;
}

export function cookiesUsuarios(data: UsuariosPersistidos): CookieAttr[] {
  const token = tokenUsuarios(data);
  const parts: string[] = [];
  ...
  if (parts.length > MAX_CHUNKS) {
    throw new Error(
      "Hay demasiadas personas para guardarlas aquí. Quita alguna e intenta de nuevo.",
    );
  }
...
export function usuariosDesdeCookies(
  ...
  try {
    ...
    return parseUsuariosPersistidos(JSON.parse(json));
  } catch (error) {
    console.error("usuarios cookie decode failed", error);
    return null;
  }
}
...
async function escribirBlob(data: UsuariosPersistidos): Promise<boolean> {
  ...
  try {
    ...
    console.error("usuarios blob put failed", error);
    return false;
  }
}

async function leerBlob(): Promise<UsuariosPersistidos | null> {
  ...
  try {
    ...
    return parseUsuariosPersistidos(await res.json());
  } catch (error) {
    console.error("usuarios blob get failed", error);
    return null;
  }
}
...
async function escribirKv(data: UsuariosPersistidos): Promise<boolean> {
  const kv = kvCreds();
  if (!kv) return false;
  try {
    ...
    console.error("usuarios kv put failed", error);
    return false;
  }
}

async function leerKv(): Promise<UsuariosPersistidos | null> {
  const kv = kvCreds();
  if (!kv) return null;
  try {
    ...
    return parseUsuariosPersistidos(
      typeof body.result === "string" ? JSON.parse(body.result) : body.result,
    );
  } catch (error) {
    console.error("usuarios kv get failed", error);
    return null;
  }
}
...
async function escribirGithub(data: UsuariosPersistidos): Promise<boolean> {
  const gh = githubMeta();
  if (!gh) return false;
  const headers = {
  ...
  try {
    ...
    const res = await fetch(
      `https://api.github.com/repos/${gh.owner}/${gh.repo}/contents/${GITHUB_FILE}`,
      {
        ...
        body: JSON.stringify({
          message: "chore: persistir personas",
    ...
    console.error("usuarios github put failed", error);
    return false;
  }
}

async function leerGithub(): Promise<UsuariosPersistidos | null> {
  const gh = githubMeta();
  ...
  try {
    ...
    console.error("usuarios github get failed", error);
    return null;
  }
}

async function escribirHttp(data: UsuariosPersistidos): Promise<boolean> {
  const url = process.env.USUARIOS_STORE_URL?.trim();
  if (!url) return false;
  const token = process.env.USUARIOS_STORE_TOKEN?.trim();
  try {
    ...
    console.error("usuarios http put failed", error);
    return false;
  }
}

async function leerHttp(): Promise<UsuariosPersistidos | null> {
  const url = process.env.USUARIOS_STORE_URL?.trim();
  if (!url) return null;
  const token = process.env.USUARIOS_STORE_TOKEN?.trim();
  try {
    ...
    console.error("usuarios http get failed", error);
    return null;
  }
}

function masReciente(
  a: UsuariosPersistidos | null,
  b: UsuariosPersistidos | null,
): UsuariosPersistidos | null {
  if (!a) return b;
  if (!b) return a;
...
export async function leerUsuariosDuraderos(): Promise<UsuariosPersistidos | null> {
...
export async function guardarUsuariosDuraderos(
  data: UsuariosPersistidos,
): Promise<{ vias: string[]; persistio: boolean }> {
  const vias: string[] = [];
  try {
    escribirUsuariosArchivo(archivoUsuariosLocal(), data);
    vias.push("archivo");
  } catch (error) {
    if (!process.env.VERCEL) throw error;
    console.error("usuarios local write failed", error);
  }
...
export function mejorUsuarios(
  ...cands: Array<UsuariosPersistidos | null | undefined>
): UsuariosPersistidos | null {
  let mejor: UsuariosPersistidos | null = null;
