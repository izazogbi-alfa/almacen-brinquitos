...
4. **Capturar las elegidas** abre una **hoja solo de captura** (pantalla completa, teléfono primero): color y talla, cantidad grande, **teclado numérico** abajo y **Enter** (siguiente talla). **Saltar color** no guarda y pasa al siguiente. **Regresar color** vuelve al color anterior si te pasaste. **Pendiente guardar** deja el registro **EN CURSO** en **Registros → Existencias pendientes**. **Terminar guardar** lo archiva en **Existencias ya terminadas**. Tocar otro color también guarda el actual. Ya no hay **Sacar**.
